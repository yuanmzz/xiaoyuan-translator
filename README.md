...
- **复制即现**：微信聊天、企业软件等无障碍不支持的地方，选中后按 `Ctrl+C`，牛来同样在光标处弹出（只观察序列号、不写剪贴板）
...
- **选中没反应？** 浏览器里请确保真正选中（蓝底）而非单击；微信聊天、企业软件等无障碍不支持的地方，选中后按一次 `Ctrl+C` 即可。CAD 画布是图形区、无文本选区所以不会弹牛来（也不会触发 COPYCLIP），CAD 文字编辑框内选中则正常翻译。若目标软件是管理员权限运行的，右键托盘 → 以管理员身份重启。
